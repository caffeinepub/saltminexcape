...
const FLOOR_H = 8;
const PLATFORM_H = 8;
function generateRandomPlatforms(levelIdx) {
  const plats = [];
...
function generateRowPlatforms(plats, rowY, W2, h, numSegs) {
  ...
  const positions = [];
  let cursor = 0;
  const leadGap = Math.random() < 0.5 ? minGap + Math.floor(Math.random() * 40) : 0;
  cursor = leadGap;
  for (let i = 0; i < numSegs; i++) {
    const remaining = W2 - cursor;
    const segsLeft = numSegs - i;
    const gapsLeft = segsLeft - 1;
    const trailGap = i === numSegs - 1 ? Math.random() < 0.5 ? minGap + Math.floor(Math.random() * 30) : 0 : 0;
    const minNeeded = segsLeft * minSegW + gapsLeft * minGap + trailGap;
    const maxSegW = remaining - minNeeded + minSegW;
    const segW = minSegW + Math.floor(Math.random() * Math.max(1, maxSegW - minSegW + 20));
    ...
    if (cursor + finalW <= W2) {
      ...
      cursor += finalW;
      if (i < numSegs - 1) {
        const maxG = remaining - finalW - (segsLeft - 1) * minSegW - (gapsLeft - 1) * minGap;
        const gapW = minGap + Math.floor(Math.random() * Math.max(1, maxG - minGap));
  ...
  for (const pos of positions) {
    if (pos.w > 0 && pos.x + pos.w <= W2 + 5) {
      plats.push({ x: pos.x, y: rowY, w: Math.min(pos.w, W2 - pos.x), h });
    }
  }
}
const LEVELS = [
...
function createSFX(audioCtx, getVolume) {
  ...
  function playTone(freq, endFreq, duration, type, gainPeak, startTime = audioCtx.currentTime) {
  ...
  function playNoise(duration, gainPeak, lowpass = 4e3, startTime = audioCtx.currentTime) {
...
function initLevel(gs, levelIdx) {
  ...
  gs.currentPlatforms = generateRandomPlatforms();
  const placementPlats = gs.currentPlatforms.filter(
    (p) => p.y < CANVAS_H - 30 && p.w > 20
  );
  const numBottles = 2 + levelIdx;
  gs.waterBottles = [];
  const usedPlats = /* @__PURE__ */ new Set();
...
function triggerBearStun(gs) {
  gs.bearStunTimer = 120;
  gs.bearRageCount++;
...
function updateGame(gs, _dt) {
  ...
  const walkSpeed = gs.debuffTimer > 0 ? BASE_WALK * 0.5 : gs.saltMeter >= 100 ? BASE_WALK * 0.6 : BASE_WALK;
  ...
  const saltSpeedMult = gs.saltMeter > 85 ? 0.25 : gs.saltMeter > 70 ? 0.55 : 1;
  const dodgeSpeedBonus = gs.dodgeBoostTimer > 0 ? 1.3 : 1;
  const speedBootsBonus = gs.speedBootsTimer > 0 ? 1.4 : 1;
  if (left) {
  ...
  if (gs.bearStunTimer === 0 && gs.bearThrowTimer >= bearThrowInterval) {
    ...
    const hw = hType === "cart" ? 32 : hType === "redCandle" || hType === "greenCandle" ? 18 : hType === "boulder" ? 20 : hType === "chest" ? 22 : hType === "pickaxe" ? 20 : hType === "saltbag" ? 20 : hType === "barrel" ? 16 : 14;
    const hh = hType === "cart" ? 22 : hType === "redCandle" || hType === "greenCandle" ? 24 : hType === "boulder" ? 20 : hType === "chest" ? 18 : hType === "pickaxe" ? 20 : hType === "saltbag" ? 22 : hType === "barrel" ? 18 : 18;
    const throwDir = Math.random() < 0.5 ? -1 : 1;
    const throwVx = throwDir * (1.5 + Math.random() * 1 + gs.level * 0.2);
    const fallsThrough = hType === "redCandle" && Math.random() < 0.5 || hType === "greenCandle" && Math.random() < 0.3;
    gs.hazards.push({
      id: gs.nextHazardId++,
      type: hType,
      x: bearCx - hw / 2,
  ...
  for (let i = 0; i < gs.hazards.length; i++) {
        ...
        if (h.x + h.w > plat.x && h.x < plat.x + plat.w) {
          ...
          if (h.vy >= 0 && prevB <= plat.y + 2 && h.y + h.h >= plat.y) {
            ...
            if (!h.wasOnGround) {
              ...
              const slowMult = h.slowTimer && h.slowTimer > 0 ? 0.3 : 1;
    ...
    if (dist < 24 && p.hitTimer === 0 && h.type !== "chest" && h.type !== "greenCandle" && h.type !== "redCandle") {
      gs.saltMeter = Math.min(100, gs.saltMeter + 0.3);
    }
    if (dist < 40 && dist > 18 && p.hitTimer === 0 && h.type !== "greenCandle" && h.type !== "redCandle") {
      if (!h.onGround) {
        gs.dodgeStreak++;
    ...
    if (rectOverlap(p.x + 2, p.y + 2, p.w - 4, p.h - 4, h.x, h.y, h.w, h.h)) {
      ...
      if (gs.currentWeapon === "pickaxe" && p.attackTimer > 0 && p.hitTimer === 0) {
        gs.portfolioValue += 150;
        ...
        if (gs.hasBubble) {
          ...
          for (let sp = 0; sp < 18; sp++) {
            ...
            gs.sparkParticles.push({
              ...
              alpha: 1,
              color: sp % 3 === 0 ? "#ffffff" : sp % 3 === 1 ? "#88ddff" : "#aaffff"
            });
          }
          p.hitTimer = 30;
          toRemove.push(i);
  ...
  for (const ka of gs.waterBottles) {
    if (!ka.collected && rectOverlap(p.x, p.y, p.w, p.h, ka.x, ka.y, 12, 18)) {
      ...
      if (totalBottlesForLevel > 0) {
        gs.saltFillReduction += (0.064 + (gs.level - 1) * 0.015) / totalBottlesForLevel;
      }
      gs.comboTimer = 90;
      gs.convictionCombo++;
  ...
  for (let spi = gs.spawnedPickups.length - 1; spi >= 0; spi--) {
    ...
    if (rectOverlap(p.x, p.y, p.w, p.h, sp2.x, sp2.y, 18, 18)) {
      ...
      if (isWeapon) {
        gs.currentWeapon = sp2.type;
        gs.weaponTimer = sp2.type === "dynamite" || sp2.type === "hardhat" ? 9999 : 600;
        const labels = {
          pickaxe: "PICKAXE!",
          gun: "GUN!",
          dynamite: "DYNAMITE!",
  ...
  if (gs.currentWeapon && gs.currentWeapon !== "dynamite" && gs.currentWeapon !== "hardhat") {
    gs.weaponTimer--;
    if (gs.weaponTimer <= 0) {
      gs.currentWeapon = null;
      gs.weaponTimer = 0;
  ...
  const flaggedToRemove = [];
  for (let i = 0; i < gs.hazards.length; i++) {
    const h = gs.hazards[i];
    if (!h.flagged || h.flagTimer === void 0) continue;
    h.flagTimer--;
    if (h.flagTimer <= 0) {
  ...
  }
  {
    ...
    gs.ambientParticles = gs.ambientParticles.filter(
      (ap) => ap.alpha > 0 && ap.y > -20 && ap.y < 680 && ap.x > -10 && ap.x < 500
    );
    for (const ap of gs.ambientParticles) {
      ap.x += ap.vx;
      ap.y += ap.vy;
      if (ap.type === "dust" || ap.type === "ash") ap.alpha -= 3e-3;
      else if (ap.type === "drip") ap.alpha -= 8e-3;
      else ap.alpha -= 5e-3;
    }
  }
  const exit = level.exitPosition;
  if (!gs.levelComplete && p.y + p.h <= exit.y + 15 && rectOverlap(p.x, p.y, p.w, p.h, exit.x - 16, exit.y - 16, 36, 36)) {
    const allBottlesCollected = gs.waterBottles.every((k) => k.collected);
    if (allBottlesCollected) {
      gs.levelComplete = true;
      gs.portfolioValue += 1e3;
...
function drawPlayer(ctx, p, saltMeter, currentWeapon, weaponTimer = 600, dyingTimer = 0, hasBubble = false, speedBootsTimer = 0, magnetTimer = 0, portfolioShieldTimer = 0, liteMode = false) {
  ...
  if (state === "run") {
    ...
    if (!liteMode) {
      ...
      ctx.globalAlpha = hitTimer > 0 && Math.floor(hitTimer / 4) % 2 === 0 ? 0.3 : 1;
    }
    ctx.fillStyle = shirtBase;
    ...
    if (saltMeter > 40) {
      ...
      ctx.globalAlpha = sweatAlpha * (0.5 + Math.sin(Date.now() * 8e-3) * 0.3);
    ...
    const idleBob = state === "idle" ? Math.sin(Date.now() * 2e-3) * 1.2 : 0;
    ...
    if (saltMeter > 50) {
      ...
      ctx.globalAlpha = sweatAlpha * (0.6 + Math.sin(Date.now() * 6e-3) * 0.3);
  ...
  if (hasGun && p.state !== "dying" && p.state !== "attacking") {
    ...
    if (!gunFlash) {
      const shouldShowGun = gunTimer > 120 || Math.floor(gunTimer / 10) % 2 === 0;
      if (shouldShowGun) {
        ctx.save();
  ...
  if (speedBootsTimer > 0 && p.state === "run") {
    const bootFlash = speedBootsTimer < 120 && Math.floor(speedBootsTimer / 8) % 2 === 0;
    if (!bootFlash) {
      ctx.save();
      ctx.globalAlpha = 0.7;
  ...
  if (portfolioShieldTimer > 0 && p.state !== "dying") {
    const shieldFlash = portfolioShieldTimer < 120 && Math.floor(portfolioShieldTimer / 8) % 2 === 0;
    if (!shieldFlash) {
      ctx.save();
      ctx.globalAlpha = 0.4;
  ...
  if (hasBubble && p.state !== "dying") {
    const bubbleAlpha = 0.25 + Math.sin(Date.now() * 6e-3) * 0.1;
    ctx.save();
...
function drawBear(ctx, bx, by, armRaise, throwAnim = 0, walkFrame = 0, tauntTimer = 0, stunTimer = 0) {
  ctx.save();
  ctx.translate(bx, by);
  if (tauntTimer > 0) {
  ...
  const breathBob = throwAnim === 0 && walkFrame < 2 ? Math.sin(Date.now() * 3e-3) * 1.5 : 0;
  ...
  if (throwAnim > 0) {
    ...
    ctx.fillStyle = "#8B0000";
    ctx.fillRect(-3, 24 + raiseL, 6, 8);
    ctx.fillStyle = "#6B0000";
    ctx.fillRect(34, 16 + raiseR, 10, 16);
    ctx.fillStyle = "#8B0000";
    ctx.fillRect(35, 24 + raiseR, 6, 8);
    ctx.restore();
  }
...
function drawHazard(ctx, h) {
  ...
  if (h.type === "boulder" || h.type === "cart" || h.type === "saltbag" || h.type === "barrel") {
  ...
  switch (h.type) {
    ...
    case "redCandle": {
      ...
      const flameFlicker = Math.sin(Date.now() * 0.02) * 2 + Math.sin(Date.now() * 0.051) * 1;
      ctx.shadowColor = "#ff4400";
      ctx.shadowBlur = 10 + Math.abs(flameFlicker) * 3;
      ctx.fillStyle = "#ffdd00";
      ctx.fillRect(h.w / 2 - 3, -4 + flameFlicker, 6, 5);
      ctx.fillStyle = "#ff8800";
      ctx.fillRect(h.w / 2 - 2, -7 + flameFlicker, 4, 4);
      ctx.fillStyle = "#ffeeaa";
      ctx.fillRect(h.w / 2 - 1, -10 + flameFlicker, 2, 4);
      ctx.fillStyle = "#ffffff";
      ctx.globalAlpha = 0.7;
    ...
    case "greenCandle": {
      ...
      ctx.fillStyle = "#222222";
      ctx.fillRect(h.w / 2 - 1, 1, 3, 5);
      const greenFlameFlicker = Math.sin(Date.now() * 0.02) * 2 + Math.sin(Date.now() * 0.047) * 1;
      ctx.shadowColor = "#00ff66";
      ctx.shadowBlur = 12 + Math.abs(greenFlameFlicker) * 3;
      ctx.fillStyle = "#ffdd00";
      ctx.fillRect(h.w / 2 - 3, -4 + greenFlameFlicker, 6, 5);
      ctx.fillStyle = "#ffaa00";
      ctx.fillRect(h.w / 2 - 2, -7 + greenFlameFlicker, 4, 4);
      ctx.fillStyle = "#aaffcc";
      ctx.fillRect(h.w / 2 - 1, -10 + greenFlameFlicker, 2, 4);
      ctx.fillStyle = "#ffffff";
      ctx.globalAlpha = 0.7;
...
function drawPlatform(ctx, x, y, w, h, tint, level = 0) {
...
function drawExit(ctx, x, y, frame) {
  ...
  ctx.fillStyle = "#ffdd00";
  ctx.fillRect(x + 5, flameY + 10, 9, 6);
  ctx.fillStyle = "#ff8800";
  ctx.fillRect(x + 6, flameY + 5, 7, 6);
  ctx.fillStyle = "#ffffaa";
  ctx.fillRect(x + 7, flameY, 5, 6);
  ctx.fillStyle = "#ffffff";
  ctx.globalAlpha = 0.6;
  ctx.fillRect(x + 7, flameY + 2, 2, 3);
  ctx.globalAlpha = 1;
...
function drawWaterBottle(ctx, x, y, frame) {
...
function drawMaturityBag(ctx, x, y, lifetime, _maxLifetime, _amount) {
  if (lifetime < 120 && Math.floor(lifetime / 10) % 2 === 0) return;
  const bob = Math.sin(Date.now() * 6e-3) * 2;
...
function drawCommunityBag(ctx, x, y, lifetime) {
  if (lifetime < 120 && Math.floor(lifetime / 10) % 2 === 0) return;
  const bob = Math.sin(Date.now() * 5e-3) * 3;
...
function drawAxePickup(ctx, x, y, frame) {
...
function drawGunPickup(ctx, x, y, frame) {
...
function drawDynamitePickup(ctx, x, y, frame) {
  const bob = Math.sin(frame * 0.08) * 3;
  ctx.save();
  ctx.translate(x, y + bob);
  ctx.shadowColor = "#ff4400";
...
function drawSaltShakerPickup(ctx, x, y, frame) {
  const bob = Math.sin(frame * 0.09) * 3;
  ctx.save();
  ctx.translate(x, y + bob);
  ctx.shadowColor = "#88ddff";
...
function drawHardHatPickup(ctx, x, y, frame) {
...
function drawBubblePickup(ctx, x, y, frame) {
...
function drawSpeedBootsPickup(ctx, x, y, frame) {
  const bob = Math.sin(frame * 0.1) * 3;
  ctx.save();
  ctx.translate(x, y + bob);
...
function drawMagnetPickup(ctx, x, y, frame) {
  const bob = Math.sin(frame * 0.08) * 3;
  ctx.save();
  ctx.translate(x, y + bob);
  ctx.shadowColor = "#cc44ff";
...
function drawPortfolioShieldPickup(ctx, x, y, frame) {
  const bob = Math.sin(frame * 0.09) * 3;
  ctx.save();
  ctx.translate(x, y + bob);
  ctx.shadowColor = "#ffd700";
...
function drawSpawnedPickup(ctx, sp, frame) {
...
function drawDynamiteInFlight(ctx, dyn, frame) {
  ctx.save();
...
function drawBackground(ctx, level, frame, liteMode = false) {
  ...
  if (!liteMode) {
    ...
    for (const torch of torchPositions) {
      const flicker = 0.7 + Math.sin(Date.now() * 8e-3 + torch.x) * 0.3;
...
function drawIntroScreen(ctx, page, frame) {
  ...
  if (Math.floor(frame / 30) % 2 === 0) {
    ...
    ctx.fillStyle = "#ffd700";
    ctx.fillText(
      page < INTRO_PAGES.length - 1 ? "PRESS SPACE TO CONTINUE" : "PRESS SPACE TO BEGIN",
      CANVAS_W / 2,
      CANVAS_H - 60
    );
  }
...
function drawYearCompleteScreen(ctx, level, score, frame) {
...
function drawGameOverScreen(ctx, score, frame) {
...
function drawVictoryScreen(ctx, score, lives, frame) {
...
function renderGame(ctx, gs, liteMode = false) {
  ctx.save();
  ...
  for (const wb of gs.waterBottles) {
    if (!wb.collected) {
      ...
      ctx.shadowBlur = 10 + Math.sin(Date.now() * 5e-3) * 4;
  ...
  for (const h of gs.hazards) {
    drawHazard(ctx, h);
    if (h.flagged && h.flagTimer !== void 0) {
  ...
  for (const mb of gs.maturityBags) {
    if (!mb.collected) {
      ctx.save();
      ctx.shadowColor = "#ffd700";
      ctx.shadowBlur = 6 + Math.sin(Date.now() * 8e-3 + mb.id) * 4;
  ...
  for (const cb of gs.communityBags) {
    if (!cb.collected) {
      ctx.save();
      const pulseR = 14 + Math.sin(Date.now() * 5e-3) * 4;
      ctx.shadowColor = "#ffd700";
      ctx.shadowBlur = 20;
      ctx.beginPath();
  ...
  for (const ft of gs.floatingTexts) {
    ...
    const scale = ft.timer > 75 ? 1.5 - (1 - ft.timer / 90) * 0.5 / (15 / 90) : 1;
    ctx.save();
    ctx.globalAlpha = alpha;
    ctx.shadowColor = ft.color;
    ctx.shadowBlur = ft.color === "#ffd700" ? 10 : ft.color === "#ff4444" ? 8 : ft.color === "#00ff88" ? 10 : 6;
...
function Game() {
  const liteModeRef = reactExports.useRef(
    /Tesla|QtWebEngine|CrMo/.test(navigator.userAgent) || /iPhone|iPad|iPod|Android/.test(navigator.userAgent)
  );
  ...
  const prevScreenRef = reactExports.useRef("TITLE");
  const [leaderboard, setLeaderboard] = reactExports.useState(() => {
    return [];
  });
  const [showNameEntry, setShowNameEntry] = reactExports.useState(false);
  const [pendingScore, setPendingScore] = reactExports.useState(0);
  const [nameInput, setNameInput] = reactExports.useState("");
  const [gamepadConnected, setGamepadConnected] = reactExports.useState(false);
  const gamepadIndexRef = reactExports.useRef(-1);
  const [hasAxeDisplay, setHasAxeDisplay] = reactExports.useState(false);
  const hasAxeDisplayRef = reactExports.useRef(false);
  const [hasGunDisplay, setHasGunDisplay] = reactExports.useState(false);
  const hasGunDisplayRef = reactExports.useRef(false);
  const prevBtnARef = reactExports.useRef(false);
  const sfxRef = reactExports.useRef(null);
  const sfxAudioCtxRef = reactExports.useRef(null);
  const [sfxMuted, setSfxMuted] = reactExports.useState(false);
  const [sfxVolume, setSfxVolume] = reactExports.useState(0.5);
  const sfxVolumeRef = reactExports.useRef(0.5);
  const sfxMutedRef = reactExports.useRef(false);
  const musicRef = reactExports.useRef(null);
  const musicStartedRef = reactExports.useRef(false);
  const [_musicMuted, _setMusicMuted] = reactExports.useState(false);
  const musicMutedRef = reactExports.useRef(false);
  const [hudData, setHudData] = reactExports.useState({
    lives: 3,
    level: 0,
    portfolioValue: 1e4,
    saltMeter: 0,
  ...
  const toggleSfxMute = reactExports.useCallback(() => {
    setSfxMuted((prev) => {
      const next = !prev;
  ...
  const handleSfxVolume = reactExports.useCallback(
    (e) => {
  ...
  const submitScore = reactExports.useCallback(() => {
  ...
  const handleSpace = reactExports.useCallback(() => {
    const gs = gsRef.current;
    if (gs.screen === "TITLE") {
      gs.screen = "INTRO";
      gs.introPage = 0;
  ...
  reactExports.useEffect(() => {
    const onKeyDown = (e) => {
      ...
      if (e.code === "Space" || e.code === "KeyZ") {
        if (["TITLE", "INTRO", "YEAR_COMPLETE", "GAME_OVER", "VICTORY"].includes(
  ...
  reactExports.useEffect(() => {
    ...
    function loop(ts) {
      ...
      if (liteModeRef.current) {
        ...
        if (now2 - lastFrameTimeRef.current < 33) {
      ...
      const gp = gamepadIndexRef.current >= 0 ? gamepads[gamepadIndexRef.current] : null;
      if (gp) {
        const gs2 = gsRef.current;
        const axisX = gp.axes[0] ?? 0;
        const axisY = gp.axes[1] ?? 0;
        const dLeft = ((_a2 = gp.buttons[14]) == null ? void 0 : _a2.pressed) || axisX < -0.5;
        const dRight = ((_b2 = gp.buttons[15]) == null ? void 0 : _b2.pressed) || axisX > 0.5;
        const dUp = ((_c2 = gp.buttons[12]) == null ? void 0 : _c2.pressed) || axisY < -0.5;
        const dDown = ((_d2 = gp.buttons[13]) == null ? void 0 : _d2.pressed) || axisY > 0.5;
        const btnA = ((_e2 = gp.buttons[0]) == null ? void 0 : _e2.pressed) ?? false;
        const btnB = ((_f2 = gp.buttons[1]) == null ? void 0 : _f2.pressed) ?? false;
        if (dLeft) gs2.keys.add("ArrowLeft");
        else gs2.keys.delete("ArrowLeft");
        if (dRight) gs2.keys.add("ArrowRight");
        else gs2.keys.delete("ArrowRight");
        if (dUp) gs2.keys.add("ArrowUp");
        else gs2.keys.delete("ArrowUp");
        if (dDown) gs2.keys.add("ArrowDown");
        else gs2.keys.delete("ArrowDown");
        if (btnA) gs2.keys.add("Space");
        else gs2.keys.delete("Space");
        if (btnB) gs2.keys.add("AxeSmash");
        else gs2.keys.delete("AxeSmash");
  ...
  const onTouchBtn = reactExports.useCallback(
    (key, active) => {
      const gs = gsRef.current;
      if (active) {
        gs.keys.add(key);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            children: [
                ...
                "div",
                {
                  ...
                  children: [1, 2, 3].map((n) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                    "svg",
                    {
                      ...
                      children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "2", y: "1", width: "3", height: "2", fill: "#ff3333" }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "7", y: "1", width: "3", height: "2", fill: "#ff3333" }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "1", y: "3", width: "10", height: "4", fill: "#ff3333" }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "2", y: "7", width: "8", height: "2", fill: "#ff3333" }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "3", y: "9", width: "6", height: "2", fill: "#ff3333" }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "4", y: "11", width: "4", height: "1", fill: "#ff3333" }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "5", y: "12", width: "2", height: "1", fill: "#ff3333" })
                      ]
                    },
                    n
                  ))
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", alignItems: "center", gap: 3 }, children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { "aria-hidden": "true", width: "10", height: "18", viewBox: "0 0 16 22", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "4", y: "0", width: "8", height: "5", fill: "#0288d1" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "3", y: "6", width: "10", height: "13", fill: "#b3e5fc" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "2", y: "8", width: "12", height: "9", fill: "#b3e5fc" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "4", y: "4", width: "8", height: "5", fill: "#b3e5fc" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: "3", y: "9", width: "10", height: "6", fill: "#ffffff" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "rect",
                    {
                      x: "4",
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { style: { color: "#81d4fa", fontSize: 10 }, children: [
                  hudData.totalBottles - hudData.bottlesLeft,
                  "/",
                  hudData.totalBottles
                ] })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", alignItems: "center", gap: 3 }, children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { color: "#aaa", fontSize: 9 }, children: "☠" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "div",
                  {
                    style: {
                    ...
                    children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "div",
                      {
                        style: {
                          ...
                          height: "100%",
                          background: hudData.saltMeter > 75 ? "#f44336" : hudData.saltMeter > 40 ? "#ff9800" : "#4caf50",
              ...
              hudData.currentWeapon && hudData.currentWeapon !== "dynamite" && hudData.currentWeapon !== "hardhat" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", alignItems: "center", gap: 3 }, children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { fontSize: 9 }, children: hudData.currentWeapon === "pickaxe" ? "⛏" : hudData.currentWeapon === "gun" ? "🔫" : hudData.currentWeapon === "saltshaker" ? "🧂" : "🔧" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "div",
                  {
                    style: {
                    ...
                    children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "div",
                      {
                        style: {
                          ...
                          height: "100%",
                          background: hudData.weaponTimer < 120 ? Math.floor(Date.now() / 200) % 2 === 0 ? "#ffd700" : "#ff4444" : hudData.currentWeapon === "gun" ? "#00ff88" : hudData.currentWeapon === "saltshaker" ? "#88ddff" : "#ffd700"
                        }
                      }
                    )
                  }
              ...
              hudData.currentWeapon === "dynamite" && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { fontSize: 9, color: "#ff6600" }, children: "💣 TNT" }),
              hudData.currentWeapon === "hardhat" && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { fontSize: 9, color: "#ffdd00" }, children: "🪖 HAT" }),
              hudData.hasBubble && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { fontSize: 9 }, children: "🫧" }),
              hudData.speedBootsTimer > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx(
                "span",
                {
                  style: {
                    fontSize: 9,
              ...
              hudData.magnetTimer > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx(
                "span",
                {
                  style: {
                    fontSize: 9,
              ...
              hudData.portfolioShieldTimer > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx(
                "span",
                {
                  style: {
                    fontSize: 9,
              ...
              liteMode && /* @__PURE__ */ jsxRuntimeExports.jsx(
                "div",
                {
                  style: {
                    color: "#888",
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                ...
                {
                  style: {
                    marginLeft: liteMode ? void 0 : "auto",
                    display: "flex",
                    gap: 4,
                    alignItems: "center"
                  },
                  children: [
                    !sfxMuted && /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "input",
                      {
                        type: "range",
                        min: "0",
                      ...
                      "button",
                      {
                        ...
                        children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                          "svg",
                          {
                            ...
                            viewBox: "0 0 22 22",
                            fill: "none",
                            children: sfxMuted ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
                              /* @__PURE__ */ jsxRuntimeExports.jsx(
                                "circle",
                                {
                                  cx: "11",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { position: "relative", lineHeight: 0 }, children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          "canvas",
          {
            ref: canvasRef,
            width: 320,
        ...
        !gamepadConnected && /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            style: {
              width: 320,
                  ...
                  "button",
                  {
                    ...
                    children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "svg",
                      {
                        ...
                        height: "22",
                        viewBox: "0 0 16 16",
                        children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M14,2 L2,8 L14,14 Z", fill: "rgba(255,255,255,0.9)" })
                      }
                    )
                  }
                ),
                  ...
                  "button",
                  {
                    ...
                    children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "svg",
                      {
                        ...
                        height: "22",
                        viewBox: "0 0 16 16",
                        children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M2,2 L14,8 L2,14 Z", fill: "rgba(255,255,255,0.9)" })
                      }
                    )
                  }
                )
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "button",
                  {
                    ...
                    style: {
                      width: 60,
                      height: 44,
                      background: hasAxeDisplay ? "rgba(255,200,50,0.25)" : hasGunDisplay ? "rgba(0,255,136,0.15)" : "rgba(255,255,255,0.1)",
                      ...
                      justifyContent: "center",
                      gap: 4,
                      color: hasAxeDisplay ? "#ffc832" : hasGunDisplay ? "#00ff88" : "rgba(255,255,255,0.4)",
                      fontFamily: "monospace",
                      fontSize: 12,
                      fontWeight: "bold",
                      touchAction: "none",
                    ...
                    children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { "aria-hidden": true, children: hasGunDisplay ? "🔫" : hasAxeDisplay ? "⛏" : "B" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { "aria-hidden": true, children: hasAxeDisplay || hasGunDisplay ? "B" : "" })
                    ]
                  }
                ),
                /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "button",
                  {
                    ...
                    children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { "aria-hidden": true, style: { fontSize: 14 }, children: "▲" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { "aria-hidden": true, children: "A" })
                    ]
                  }
                )
              ] })
        ...
        showNameEntry && /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
                ...
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "div",
                    {
                      ...
                      children: gsRef.current.screen === "VICTORY" ? "YOU ESCAPED!" : "GAME OVER"
                    }
                  ),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { fontSize: 12, color: "#aaa" }, children: [
                    "PORTFOLIO: $",
                    pendingScore.toLocaleString()
                  ] }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { fontSize: 13, color: "#ffd700", marginTop: 8 }, children: "⛏ ENTER YOUR NAME (optional)" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "input",
                    {
                      maxLength: 12,
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { fontSize: 10, color: "#888", textAlign: "center" }, children: "Leave blank to submit as James Salt" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "button",
                    {
                      type: "button",
                  ...
                  leaderboard.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { fontSize: 11, color: "#555" }, children: "No scores yet. Be the first!" }),
                  leaderboard.slice(0, 10).map((entry, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                    "div",
                    {
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { fontSize: 10, color: "#555", marginTop: 8 }, children: "PRESS SPACE TO PLAY AGAIN" })
                ]
              }
            )
          }
